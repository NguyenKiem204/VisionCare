...
const MedicalRecordsTable = ({ medicalRecords, loading, onEdit, onDelete }) => {
  ...
  if (!medicalRecords || medicalRecords.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow">
        <div className="p-8 text-center">
          <p className="text-gray-500 dark:text-gray-400">Không có hồ sơ bệnh án nào</p>
        </div>
      </div>
    );
  }
              ...
              <tr key={record.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                  <div className="flex flex-col">
                    <span>Trái: {formatVision(record.visionLeft)}</span>
                    <span>Phải: {formatVision(record.visionRight)}</span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
