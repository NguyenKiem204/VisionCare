...
export default function BlogEditor() {
  ...
  // Image will be uploaded when submitting the form, no need for separate upload
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await updateBlog(id, formData, imageFile);
        toast.success("Cập nhật blog thành công");
      } else {
        const response = await createBlog(formData, imageFile);
        toast.success("Tạo blog thành công");
        if (response.success && response.data) {
          navigate(`/admin/blogs/${response.data.blogId}/edit`);
          return;
        }
      }
      navigate("/admin/blogs");
